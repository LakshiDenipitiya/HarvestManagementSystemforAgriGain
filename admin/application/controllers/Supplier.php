...
class Supplier extends CI_Controller{
    ...
    {
        parent::__construct();
        $this->load->model("Supplier_Model"); //load supplier model
        $this->load->model("Location_Model"); //load location model
         $this->load->model("Item_Model"); //load item model
         $this->load->model("Supplieritem_Model"); // laod supplier item model
 ...
 //load to supplier_list all data from the supplier table
        ...
        foreach ($supplier_list as $key => $sup) {
            $sup->items = $this->Supplieritem_Model->get_items_by_supplier($sup->id);  
        }
    ...
    public function create()
    {
        ...
        $this->isLoggedIn();
         ...
         if (!$this->permissions->has_permission('supplier_create', $this->allowedPermissionCodes)) {
           $this->layouts->view("Nopermissions/index");
            return;
        }
    ...
    function update()
    {
        ...
        {
            $result = $this->Supplier_Model->update_status($id, $status);

            if($result) {
                $this->session->set_flashdata('message','<div class="alert alert-success alert-dismissible">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                    <h4><i class="icon fa fa-check"></i> Supplier status updated successfully!</h4></div>');
            }else{
